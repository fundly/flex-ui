package com.enilsson.elephanttrakker.commands
{
	...
	public class LoginCommand extends SequenceCommand implements ICommand, IResponder
	{
		...
		private function login(event:LoginEvent):void
		{
			...
			_model.login.ieDebug += '<br>--Start Login--<br>' + _model.authURL;
			_model.login.ieDebug += ObjectUtil.toString(event.loginAttempt);
		...
		private function onResults_login(event:Object):void 
		{
			...
			_model.login.ieDebug += '<br>--Success Login--<br>';
		...
		{
			//if(_model.debug) 
			Logger.info('Forget details Event Success', ObjectUtil.toString(event.result));
	
			if (event.result) 
			{
				_model.login.forgetErrorMessage = 'An email has been successfully sent to your account.';
				_model.login.forgetErrorVisible = true;		
				_model.login.forgetErrorStyle = "successBox";	
			} 
			else 
			{
				_model.login.forgetErrorMessage = 'Internet connection error, please check before trying again!';
				_model.login.forgetErrorVisible = true;			
				_model.login.forgetErrorStyle = "errorBox";	
			}	
		}
