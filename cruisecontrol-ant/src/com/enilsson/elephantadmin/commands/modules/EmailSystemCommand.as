package com.enilsson.elephantadmin.commands.modules
{
	...
	public class EmailSystemCommand extends SequenceCommand implements ICommand, IResponder
	{
		private var _model:EAModelLocator = EAModelLocator.getInstance();
		private var _moduleName:String = 'email_system';
		
		public function EmailSystemCommand()
		{
			super();
		}
		...
		override public function execute(event:CairngormEvent):void
		{
			...
			switch(event.type)
			{
				case EmailEvent.EMAIL_SYSTEM_LAYOUT :
				...
				break;
				case EmailEvent.EMAIL_SYSTEM_UPSERT :
					upsertRecord(event as EmailEvent);
				break;			
				case EmailEvent.EMAIL_SYSTEM_ACTIVITY :
					recordActivity(event as EmailEvent);
				break;
				case EmailEvent.EMAIL_SYSTEM_TEST :
					sendTestEmail(event as EmailEvent);
				break;
				case EmailEvent.EMAIL_SYSTEM_GET_ATTACHMENTS :
					getAttachmentList(event as EmailEvent);
				break;
			}
		}
		...
		private function getLayout(event:EmailEvent):void
		{
			...
			delegate.getLayout( new LayoutVO("email_system_templates") );
		}
		...
		private function getRecords(event:EmailEvent):void
		{
			...
			_model.dataLoading = true;
			_model.email_system.lastQuery = EmailEvent(event);
		...
		private function onResults_upsertRecord(event:Object):void 
		{
			...
			switch(event.result.state)
			{
				case '99' :
				case '98' :
					_model.errorVO = new ErrorVO('Email template edited successfully!', 'successBox', true );
		...
		private function onResults_deleteRecord(event:Object):void 
		{
			...
			{
				case '88' :
					_model.errorVO = new ErrorVO('Email tempalte deleted successfully!', 'successBox', true );
					_model[_moduleName].lastQuery.dispatch();
					_model[_moduleName].selectedIndex = _model[_moduleName].lastIndex;
				break;
				case '-88' :			
		...
		private function sendTestEmail(event:EmailEvent):void
		{
			if(_model.debug) Logger.info('Sending Test Email', ObjectUtil.toString(event.params.emailVO));		

			var handlers:IResponder = new mx.rpc.Responder(onResults_sendTestEmail, onFault_sendTestEmail);
			var delegate:PluginsDelegate = new PluginsDelegate(handlers);
			
			// show the data loading icon
			_model.dataLoading = true

			delegate.sendTestEmail(event.params.emailVO);
		}

		private function onResults_sendTestEmail(event:Object):void 
		{
			_model.dataLoading = false;
			_model.errorVO = new ErrorVO("A test email was sent to " + _model.session.email, 'successBox', true );
		}

		private function onFault_sendTestEmail(event:Object):void
		{
			if(_model.debug) Logger.info('sendTestEmail Fail',ObjectUtil.toString(event));

			_model.dataLoading = false;			
		}


		private function getAttachmentList(event:EmailEvent):void
		{
			var handlers:IResponder = new mx.rpc.Responder(onResults_getAttachmentList, onFault_getAttachmentList);
			var delegate:RecordsDelegate = new RecordsDelegate(handlers);
		...
		private function onResults_getAttachmentList(event:Object):void 
		{
			if(_model.debug){ Logger.info(_moduleName + ' getAttachmentList Success', ObjectUtil.toString(event.result)); }
			
			_model.dataLoading = false;
			
			var tableName:String = event.result.table_name;
			
			_model[_moduleName].attachmentList = new ArrayCollection();
			var firstItem:Object = new Object();
			firstItem.title = "No Attachment"
			firstItem.attachment_id = 'NULL';

			_model[_moduleName].attachmentList.addItem(firstItem);
			for each( var item:Object in event.result[tableName])
			{
				_model[_moduleName].attachmentList.addItem(item);
			}
		}
		
		public function onFault_getAttachmentList(event:Object):void
		{
			if(_model.debug) Logger.info(_moduleName + ' getRecords Fail', ObjectUtil.toString(event));		
