package com.enilsson.elephantadmin.commands.modules
{
	...
	public class NewsCommand extends SequenceCommand implements ICommand, IResponder
	{
		private var _model:EAModelLocator = EAModelLocator.getInstance();
		private var _moduleName:String = 'news';
		...
		override public function execute(event:CairngormEvent):void
		{
			...
			switch(event.type)
			{
				case NewsEvent.NEWS_LAYOUT :
					getLayout(event as NewsEvent);
				break;
				case NewsEvent.NEWS_RECORDS :
					getRecords(event as NewsEvent);
				break;
				case NewsEvent.NEWS_SEARCH :
					searchRecords(event as NewsEvent);
				break;
				case NewsEvent.NEWS_UPSERT :
					upsertRecord(event as NewsEvent);
				break;
				case NewsEvent.NEWS_DELETE :
					deleteRecord(event as NewsEvent);
				break;				
				case NewsEvent.NEWS_ACTIVITY :
					recordActivity(event as NewsEvent);
				break;				
				case NewsEvent.NEWS_EXPORT :
					exportTable(event as NewsEvent);
				break;		
			}
		}
		 ...
		 */			
		private function getLayout(event:NewsEvent):void
		{
		 ...
		 * Get the users table layout to build the form
		 */			
		private function getRecords(event:NewsEvent):void
		{
			...
			_model.dataLoading = true;
			_model.news.lastQuery = NewsEvent(event);
		 ...
		 * Get the users table layout to build the form
		 */			
		private function searchRecords(event:NewsEvent):void
		{
		 ...
		 * Upsert a record
		 */			
		private function upsertRecord(event:NewsEvent):void
		{
		 ...
		 * Delete a record
		 */			
		private function deleteRecord(event:NewsEvent):void
		{
		 ...
		 */			
		private function recordActivity(event:NewsEvent):void
		{
		...
		private function exportTable(event:NewsEvent):void
		{			
