package com.enilsson.elephantadmin.commands.modules
{
	...
	public class EmailAttachmentsCommand extends SequenceCommand implements ICommand, IResponder
	{
		private var _model:EAModelLocator = EAModelLocator.getInstance();
		private var _moduleName:String = 'email_attachments';
		
		public function EmailAttachmentsCommand()
		{
			super();
		}
		...
		override public function execute(event:CairngormEvent):void
		{
			...
			switch(event.type)
			{
				case EmailEvent.EMAIL_ATTACHMENTS_LAYOUT :
				...
				break;
				case EmailEvent.EMAIL_ATTACHMENTS_UPSERT :
					upsertRecord(event as EmailEvent);
				break;
				case EmailEvent.EMAIL_ATTACHMENTS_DELETE :
					deleteRecord(event as EmailEvent);
				break;
				case EmailEvent.EMAIL_ATTACHMENTS_ACTIVITY :
					recordActivity(event as EmailEvent);
				break;
				case EmailEvent.EMAIL_ATTACHMENTS_EXPORT :
					exportTable(event as EmailEvent);
				break;
			}
		}
		...
		private function getRecords(event:EmailEvent):void
		{
			...
			_model.dataLoading = true;
			_model.email_attachments.lastQuery = EmailEvent(event);

			if(_model.debug) Logger.info(_moduleName + ' getRecords call', ObjectUtil.toString(event.params.recordsVO));
		...
		private function exportTable(event:EmailEvent):void
		{			
